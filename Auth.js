...
/** Ganti password sendiri (semua peran). Verifikasi password lama dulu. */
function gantiPasswordSendiri(token, lama, baru) {
  var me = requireAuth(token);
  var u = findRow('Users', 'id', me.id);
  if (!u) return { success: false, message: 'Akun tidak ditemukan.' };
  if (!baru || String(baru).length < 5) return { success: false, message: 'Password baru minimal 5 karakter.' };
  if (hashPassword(String(lama || ''), u.salt) !== u.password_hash) return { success: false, message: 'Password lama salah.' };
  var salt = generateSalt();
  updateRow('Users', u._row, { salt: salt, password_hash: hashPassword(String(baru), salt) });
  logAction(u.id, 'ganti_password', 'Ganti password sendiri');
  return { success: true };
}